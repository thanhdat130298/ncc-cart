...
export default new Vuex.Store({
  ...
  actions: {
    ...
    saveCart(state, product) {
      let finder = this.state.carts.find(array => {
       return product.idPro==array.idPro
      });
      if (finder) {
        Vue.axios
          .put("http://5e6aec600f70dd001643c0b4.mockapi.io/apiv1/cart/"+this.state.cartss.qty, {
            idPro: product.idPro,
            price: product.price,
            name: product.name,
            qty: this.state.carts.qty++
          })
          // .then(result => {
          //   console.log(result)
          // })

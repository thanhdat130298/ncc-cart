...
<template>
  <div>
    <div class="container">
      ...
      <div v-for="carts in cart" v-bind:key="carts.idPro">
        <div>Product Name: {{carts.name}}</div>
        <div>Product Quantity: {{carts.quantity}}</div>
...
export default {
  ...
  methods: {
    ...
    updateCart(products) {
      ...
      this.qtyCart = products.quantity;
      if (found) {
        console.log('trung'), 
        products.quantity++;
      } else {
        this.qtyCart+1;
        this.cart.push(products);console.log('ko trung')
      }
    }
  },
  computed: {

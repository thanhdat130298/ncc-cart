...
export default {
  ...
  methods: {
    ...
    updateCart(products) {
      ...
      var temp = this.cart[this.idBut];
      
      var found = this.cart.find(array => {
        return temp.idPro == array.idPro;
      });
      if (found) {
        temp.quantity++;
      } else {
        this.cart.push({
          quantity: 1,
          idProInCart: this.product[this.idPro].idPro,
          nameProInCart: this.product[this.idPro].name,
          priceProInCart: this.product[this.idPro].price
        });
      }
      console.log(temp)

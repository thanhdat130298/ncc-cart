...
export default {
  ...
  methods: {
    total () {
      var sum = 0;
      console.log("ccc",this.$store.getters.inCart.length)
        for (let i = 0; i < this.$store.getters.inCart.length; i++) {
          sum += this.$store.getters.inCart.price * this.$store.getters.inCart.qty;
        }
        return sum;
}},
  computed: {

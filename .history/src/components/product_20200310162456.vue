...
<template>
  <div>
    <div class="container">
      ...
      <div class="cartDetail" v-for="products in product" :key="products">{{a(products)}}
      </div>
...
export default {
  ...
  methods: {
    ...
    updateCart(products) {
     ...
     // this.cart.push(arr);
      var found = this.cart.find(array => {return products.idPro == array.idPro });
      if (found) {
          products.quantity++;
          console.log('trung')
      } else {this.cart.push(products), console.log('ko trung')}
      
    },
    a(products) {
      return this.cart[products.idPro].name;
    }
  }
};
</script>
